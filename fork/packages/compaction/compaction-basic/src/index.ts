 ...
 *
 * @module @deepseek-ai/dsh-fork-compaction-basic
 */
...
import { CONTEXT_WINDOW_EXCEEDED_CODE, assertNever } from '@deepseek-ai/dsh-fork-llm'
...
import type {} from '@deepseek-ai/dsh-fork-compaction-policy'
...
export class BasicCompactionEngine extends CompactionEngine {
  ...
  override async compactIfNeeded(
    ...
    // The session's absolute cap, when one is set, binds alongside the ratio:
    // the pressure threshold is whichever constraint is smaller.
    const sessionLimit = compactionPolicy?.limitTokens?.(agent.session.id)
    const spec = resolveSessionSpec(effectivePolicy, context.contextWindow, sessionLimit)
