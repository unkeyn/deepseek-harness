 ...
 *
 * @module @deepseek-ai/dsh-fork-compaction-basic/config
 */

import { deepFreeze } from '@deepseek-ai/dsh-fork-llm'
import type { LlmCallConfig } from '@deepseek-ai/dsh-fork-llm'
import type {
...
/**
 * Fixed reserve below an absolute session cap where pressure compaction fires.
 * Compaction only runs between steps, and the summarization request itself
 * consumes cap headroom, so triggering at the exact cap lets one large step
 * overshoot the wall the user asked to stay under. Every mainstream harness
 * reserves a buffer for the same reason; 5% of even the smallest accepted cap
 * (1024 tokens) leaves ~51 tokens, while typical caps leave hundreds.
 */
const SESSION_CAP_TRIGGER_RATIO = 0.95

/**
 * Scale one routed policy into concrete token budgets for its model capacity,
 * then bind them at an optional per-session absolute cap.
 * @param policy - merged policy for the exact routed target.
 * @param contextWindow - positive adapter-owned capacity for that target.
 * @param limitTokens - absolute session cap; `undefined` leaves the scaled budgets alone.
 * @returns detached immutable pressure and retention budgets under both constraints.
 */
export function resolveSessionSpec(
  ...
  const base = resolveCompactSpec(policy, contextWindow)
  if (limitTokens === undefined) return base
  const thresholdTokens = Math.min(base.thresholdTokens, Math.floor(limitTokens * SESSION_CAP_TRIGGER_RATIO))
  // Retention must stay below whichever threshold binds. Halving the binding
  // threshold keeps a balanced reduction selectable even at the smallest
  // accepted limit, where the window-scaled retention would otherwise exceed
  // it and disable pressure compaction entirely.
  const retainTokens = Math.min(base.retainTokens, Math.floor(thresholdTokens / 2))
  return deepFreeze({ ...base, thresholdTokens, retainTokens })
}
