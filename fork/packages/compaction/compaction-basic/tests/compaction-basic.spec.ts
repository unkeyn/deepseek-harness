...
import BasicCompactionEngine from '@deepseek-ai/dsh-fork-compaction-basic'
import type { BasicCompactionConfig } from '@deepseek-ai/dsh-fork-compaction-basic'
import { selectCompactableRange } from '@deepseek-ai/dsh-fork-compaction-basic/src/region.ts'
import type { SummarizationInput, SummaryResult } from '@deepseek-ai/dsh-fork-compaction-basic/src/summarizer.ts'
...
import {
  ...
  resolveSessionSpec,
  resolveTargetPolicy,
} from '@deepseek-ai/dsh-fork-compaction-basic/src/config.ts'
import type { CompactionResult } from '@deepseek-ai/dsh-compaction'
import LlmRuntime, { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, createToolResultMessage, LlmAdapter , createMessage } from '@deepseek-ai/dsh-fork-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmFailure,
...
describe('compact configuration and defaults', () => {
  ...
  it('binds scaled budgets at a per-session absolute cap and clamps retention under it', () => {
    const policy = resolveTargetPolicy(resolveConfig({}), { provider: MODEL, model: MODEL })
    // No cap: the plain window-scaled budgets.
    expect(resolveSessionSpec(policy, 10_000, undefined)).toMatchObject({
      thresholdTokens: 8_000,
      retainTokens: 1_600,
    })
    // The cap binds below the ratio: the trigger reserves 5% of the cap so
    // between-step growth and the summarization call stay inside the limit,
    // and retention halves so a reduction stays selectable.
    expect(resolveSessionSpec(policy, 10_000, 2_000)).toMatchObject({
      thresholdTokens: 1_900,
      retainTokens: 950,
    })
    // A cap far below the window-scaled retention still leaves headroom to compact into.
    expect(resolveSessionSpec(policy, 100_000, 1_024)).toMatchObject({
      thresholdTokens: 972,
      retainTokens: 486,
    })
    // A cap above the scaled threshold changes nothing.
    expect(resolveSessionSpec(policy, 10_000, 50_000)).toMatchObject({
      thresholdTokens: 8_000,
      retainTokens: 1_600,
    })
  })
...
describe('pressure measurement and retention', () => {
  ...
  it('compacts at a per-session absolute cap below the window-scaled threshold', async () => {
    // The default window ratio leaves this conversation below pressure; only
    // the session's own smaller cap makes it qualify.
    const ctx = createContext()
    ctx.provide('compactionPolicy', {
      thresholdRatio: (fallback: number) => fallback,
      limitTokens: (sessionId: string) =>
        sessionId === 'conversation-6' ? 500 : undefined,
    })
    const compact = service({ auto: false }, ctx)

    await expect(compactIfNeeded(compact, conversation(6, 'x'.repeat(600)))).resolves.not.toBeNull()

    // A session without a cap keeps the plain window-scaled threshold.
    await expect(compactIfNeeded(compact, conversation(3, 'y'.repeat(200)))).resolves.toBeNull()
  })

  it('keeps working when the policy service predates per-session limits', async () => {
    const ctx = createContext()
    ctx.provide('compactionPolicy', { thresholdRatio: (fallback: number) => fallback })
