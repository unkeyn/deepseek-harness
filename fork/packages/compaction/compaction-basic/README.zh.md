...
- **路由策略**：主动压力从拥有最新持久提供方／模型路由的适配器解析容量，再将默认策略与可选的精确目标覆盖缩放为具体 token 预算。按会话的绝对上限（`compaction-policy` settings namespace 中属于该会话 id 的 `sessionLimits` 条目）会与窗口比例阈值一同生效：压力阈值取二者中较小者，并在上限之下保留固定的 5% 余量，使步间增长与摘要调用本身都留在限制之内；保留预算会被钳制到生效阈值的一半，因此即便在最小可用上限（1024 token）下仍能选出可压缩范围。模型发现仍仅供参考，不参与此处的策略解析。
...
- **生命周期**：所有入口点共享一个先记录标记的区域事务。它会验证范围与活动锁，同步追加 `compaction/start`，准备并等待摘要，重新验证，再追加 `compaction/summary` 和替换，最后恰好进行一次闭合尝试。自动压缩通过串行 `agent/pre-step` listener 运行：前一个步骤结束后、下一个请求组装前执行，因此替换完成后当前轮次会继续。可选的 Host `compactionPolicy` service 会覆盖已解析的阈值比例，并通过 `limitTokens(sessionId)` 提供按会话的绝对上限；service 缺席、缺少 `limitTokens` 方法或字段未设置时，保留配置中的比例。`compactNow()` 仍是仅限 idle 的显式操作，自动压力路径不会调用它。
...
`BasicCompactionEngine` 需要 `ctx.llm`、`ctx.tokenMeter` 和 `ctx.sessions`，并可选地从 [`dsh-compaction-policy`](../compaction-policy/README.md) 读取 `ctx.compactionPolicy`。以下组合从其宿主接收 `ctx.llm`，并安装另外两项服务：
