...
- **Routed policy** — proactive pressure resolves capacity from the adapter that owns the latest durable provider/model route, then scales the default policy plus an optional exact-target override into concrete token budgets. A per-session absolute cap (the `compaction-policy` settings namespace's `sessionLimits` entry for this session id) binds alongside the window-scaled threshold: the pressure threshold is whichever constraint is smaller, taken at a fixed 5% reserve below the cap so between-step growth and the summarization call itself stay inside the limit, and retention is clamped to half of that binding threshold so a reduction stays selectable even at the smallest accepted limit (1024 tokens). Model discovery remains advisory and is not consulted.
