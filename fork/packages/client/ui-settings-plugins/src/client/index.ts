...
// Type-only: the Models page's `settings.models.panel` SlotMap merge, which
// hosts this package's ready-made search-providers panel.
...
import { WEB_SEARCH_POOL_NS, WebSearchPoolCardController } from './custom-web-search-pool-controller.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const webSearchPool = new WebSearchPoolCardController(ctx.settingsScope.bind({ namespace: WEB_SEARCH_POOL_NS }), api, rpc)
  ctx.effect(() => () => { webSearchPool.dispose() }, 'ui-settings-plugins: custom web search pool')
