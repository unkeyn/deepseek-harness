...
export interface ModelsSectionInjected {
  ...
  hooks: {
    ...
    /** Ordered, locale-aware projection of the feature-panel ledger. */
...
/** One feature panel projected from a `settings.models.panel` contribution. */
export interface ModelsPanelEntry {
  /** Stable panel id; also the slot key the segment renders. */
  id: string
  /** Display order after the built-in API segment. */
  order: number
...
/**
 * Render the Models section content column: the segment switcher around the
 * provider list and every feature panel registered beside it.
 * @param props - slot-delivered injected dependencies.
 * @returns the section, or null while the shell has not injected yet.
 */
export function ModelsSection(props: ModelsSectionProps): ReactNode {
  const { controller, useSnapshot, usePanels, api, schema, t, renderSlot } = props
  if (
    controller === undefined || useSnapshot === undefined || usePanels === undefined
    || api === undefined || schema === undefined || t === undefined
    || renderSlot === undefined
  ) return null
  return <Loaded injected={{ controller, useSnapshot, usePanels, api, schema, t, renderSlot }} />
}

/** The built-in panel id owning the provider list; always the first segment. */
...
function ApiContent({ injected }: { injected: ModelsSectionFace }): ReactNode {
