...
describe('ModelsSection', () => {
  ...
  it.each([
    ...
    // stored override until the unset is applied â€” so the rows did not change
  ...
  it('keeps every row\'s unreadable text, not just the last one edited', async () => {
    ...
    // the first, which then fell back to rendering its stored NaN as `NaN` â€”
  ...
  it('drops the typed text when reset replaces the rows it annotated', async () => {
    ...
    // inherited row displayed text no settings layer stores â€” and because an
  ...
  it('keeps the card usable when the write rejects instead of answering', async () => {
    ...
    // Not stuck in `applyingâ€¦`: the finally cleared busy, so Apply is live again.
  ...
  it('collapses the setup card on cancel without disturbing another open card', async () => {
    ...
    // The add card kept its draftâ€¦
    expect(screen.getByLabelText(en.provider)).toBeTruthy()
    // â€¦and DeepSeek collapsed to an ordinary row carrying the missing-key dot.
    expect(screen.getAllByLabelText(en.keyInput)).toHaveLength(1)
...
describe('apiKeyFailure', () => {
  it('treats a blank field as no failure â€” it means keep the stored key', () => {
