...
const themeStyles = resolve(process.cwd(), 'packages/client/ui-theme/src/styles')
const tokens = readdirSync(themeStyles)
  .filter(name => name.endsWith('.css'))
  .map(name => readFileSync(resolve(themeStyles, name), 'utf8'))
  .join('\n')
