...
import type { ModelsPanelEntry, ModelsSectionInjected, ModelsSectionProps } from '../src/client/ModelsSection.tsx'
...
/** A stable empty panel ledger for lanes that register no feature panels. */
const NO_PANELS: readonly ModelsPanelEntry[] = []
const NO_PANELS_STORE = {
  getSnapshot: () => NO_PANELS,
  subscribe: () => () => {},
}
...
async function mountSection(options: Parameters<typeof scriptedFace>[0] = {}) {
  ...
  const injected: ModelsSectionProps = {
    ...
    // No feature panels are registered in this lane; the section renders the
    // provider list alone behind its built-in API segment.
...
describe('endpoint interrogation', () => {
  ...
  it('pre-checks the already-configured ids and adopts only what stays picked', async () => {
    ...
    // The dialog opens as a view of the current selection: configured rows
    // start checked, everything else waits for a deliberate pick.
  ...
  it('toggles a candidate off and back on before adopting', async () => {
    ...
    // Nothing is configured, so both start unchecked; two clicks leave the
    // first where it began, and picking the second adopts it with its
    // disclosed output cap.
    ...
    await waitFor(() => { expect(mutate).toHaveBeenCalled() })
    expect(firstMutate(mutate).ops[0]?.value).toEqual([{ id: 'b', maxTokens: 2048 }])
  })
  ...
  it('filters candidates by a fuzzy subsequence and scopes select-all to the view', async () => {
    const discover = vi.fn(() => Promise.resolve(ok({
      models: [{ id: 'openai/gpt-4o' }, { id: 'anthropic/claude-3' }, { id: 'meta/llama-3' }],
    })))
    await mountSection({ discover })
    openEditor('openai')
    ...
    fireEvent.click(screen.getByText(en.fetchModels))
    const dialog = await screen.findByRole('dialog')
    // A scattered subsequence still matches, and tighter hits rank first.
    fireEvent.change(screen.getByLabelText(en.fetchSearch), { target: { value: 'g4o' } })
    expect(screen.getByText('1 / 3')).toBeTruthy()
    let boxes = [...dialog.querySelectorAll<HTMLInputElement>('input[type="checkbox"]')]
    expect(boxes).toHaveLength(1)

    // Select-all is a statement about the filtered view only.
    fireEvent.click(within_(dialog, en.fetchSelectAll))
    fireEvent.change(screen.getByLabelText(en.fetchSearch), { target: { value: '' } })
    expect(screen.getByText('3 / 3')).toBeTruthy()
    boxes = [...dialog.querySelectorAll<HTMLInputElement>('input[type="checkbox"]')]
    expect(boxes.map(box => box.checked)).toEqual([true, false, false])

    // Clearing the field restores the provider's own listing order.
    expect([...dialog.querySelectorAll('li span')].map(span => span.textContent))
      .toEqual(['openai/gpt-4o', 'anthropic/claude-3', 'meta/llama-3'])
  })
})
