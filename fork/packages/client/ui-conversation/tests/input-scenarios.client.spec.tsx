...
const { FakeApiClient, fakeRemote, ok } = await import(resolve(process.cwd(), 'packages/client/runtime/tests/fake-api.client.ts'))
...
async function scopedBench(register?: (inputTriggers: InputTriggerService) => void) {
  ...
  const barProps: InputBarProps = {
    ...
    useSessionLimit: bindSnapshotSelector(createSnapshotStore<number | null>(null)),
