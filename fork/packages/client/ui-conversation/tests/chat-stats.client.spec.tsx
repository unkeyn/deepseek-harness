...
describe('StatsLine', () => {
  ...
  it('keeps every stats group visible for narrow composer layouts', () => {
    const { source } = makeSource({ nodes: [assistant(1, 1)] })
    const view = render(<StatsLine {...props(source)} />)
    expect(view.container.textContent).toContain('1 turns · 1 steps')
    expect(view.container.textContent).toContain('Cache hit 90%')
    expect(view.container.textContent).toContain('Input 100 tok · Output 5 tok')
  ...
  it('renders window latency and throughput beside the wall-time group', () => {
    ...
    const view = render(<StatsLine {...props(source)} />)
    expect(view.container.textContent).toContain('LLM 3.8sTTFT avg 0.8s · 20 tok/s')
  })
  ...
  it('takes every stats label from the active locale', () => {
    ...
    expect(view.container.textContent)
      .toBe('1 轮 · 1 步LLM 3.8s首 token 平均 0.8s · 20 tok/s| 缓存命中 90%输入 100 tok · 输出 5 tok')
  })
  ...
  it('swaps the billing group for the heuristic composition while the ring is hovered', () => {
    const { source } = makeSource({ nodes: [assistant(1, 1)] })
    const view = render(<StatsLine {...props(source, {
      tokenUsage: USAGE,
    ...
    expect(view.container.textContent).toContain('Cache hit 90%')
    act(() => { contextMeterHover.set(true) })
    // The strip is reused: same row, the billing figures give way to the
    // composition rows for exactly as long as the hover lasts.
    expect(view.container.textContent).not.toContain('Cache hit 90%')
    expect(view.container.textContent).toContain('System prompt ~120')
    expect(view.container.textContent).toContain('Tools ~21.5K')
    expect(view.container.textContent).toContain('Messages ~477K')
    expect(view.container.firstElementChild?.getAttribute('aria-label'))
      .toBe('1 turns · 1 steps. System prompt ~120. Tools ~21.5K. Messages ~477K')
    act(() => { contextMeterHover.set(false) })
    expect(view.container.textContent).toContain('Cache hit 90%')
    expect(view.container.textContent).not.toContain('System prompt')
  })

  it('keeps the billing group when the hover swap has no composition to show', () => {
    const { source } = makeSource({ nodes: [assistant(1, 1)] })
    const view = render(<StatsLine {...props(source)} />)
    act(() => { contextMeterHover.set(true) })
    // No contextBreakdown projection: the swap has nothing to render, so the
    // billing group stays.
    expect(view.container.textContent).toContain('Cache hit 90%')
  })
  ...
  it('renders whole-log wall times and speeds from the projection, not the loaded window', () => {
    ...
    expect(view.container.textContent).toBe(
      '200 turns · 200 stepsLLM 1m40s · Tool call 1m2sTTFT avg 0.8s · 20 tok/s| Cache hit 90%Input 100 tok · Output 5 tok',
    )
  })
