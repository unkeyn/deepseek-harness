...
/** No session, therefore no cap; same one-identity rule as above. */
const ABSENT_SESSION_LIMIT = {
  getSnapshot: (): number | null => null,
  subscribe: () => () => {},
}
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId | undefined): ComposerBarInjected => {
      if (sessionId === undefined) {
        return {
          ...
          hooks: {
            ...
            sessionLimit: ABSENT_SESSION_LIMIT,
          },
        }
      }
      const conversation = concreteConversation(ctx)
      ...
      return {
        ...
        setCompactionThreshold: (value) => { compactionPolicy.setThreshold(value) },
        setSessionLimit: (tokens) => { compactionPolicy.setSessionLimit(sessionId, tokens) },
        hooks: {
          notices: shell.notices,
          lexicon: shell.lexicon,
          menuLauncher: inputTriggers?.launcher ?? ABSENT_MENU_LAUNCHER,
          compactionThreshold: compactionPolicy.threshold,
          sessionLimit: compactionPolicy.storeFor(sessionId),
        },
      }
    },
  }, InputBar)
