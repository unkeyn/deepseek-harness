...
import type {} from '@deepseek-ai/dsh-fork-client-ui-layout/client'
...
export interface ComposerBarInjected {
  ...
  /** Set or clear this session's absolute context cap in tokens (`null` clears); absent with the session. */
  setSessionLimit: ((tokens: number | null) => void) | undefined
  /**
   * Registrant hooks compartment: the renderer binds these to
   * useNotices/useLexicon/useCompactionThreshold/useSessionLimit (static absent sources without a
   * session — hook order stays constant).
   */
  hooks: {
    ...
    sessionLimit: ObservableSnapshot<number | null>
  }
}
