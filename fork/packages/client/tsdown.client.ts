...
import { optionalStringArray } from '../../../packages/client/modules/src/client/manifest.ts'
 ...
 */
export const INLINE_SAFE = /^@deepseek-ai\/dsh-(?:fork-)?(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/
...
function browserSourcePath(source: string, sourcemapPath: string): string {
  ...
  return repositoryPath.startsWith('packages/') || repositoryPath.startsWith('fork/packages/')
    ? `../../../${repositoryPath}`
    : source
}
...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  for (const manifestPath of [
    ...
    ...globSync('fork/packages/*/*/package.json', { cwd: REPOSITORY_ROOT }),
  ]) {
