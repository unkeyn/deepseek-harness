# @deepseek-ai/dsh-fork-host-apiproxy
...
会话模型选择属于会话领域约定。`session.models` 将当前 `ModelSelection` 与按提供方分组的建议性模型、精确模型推理元数据和逐提供方查询失败记录分开返回。该选择可能不在这些分组中，也绝不会作为合成行注入；客户端可以提示用户作出另一项选择，而无需把目录变成路由白名单。`session.selectModel` 校验由适配器持有的可选推理强度，并指定下次组装提示词时使用的完整选择。待处理的图片提示词在发布或被丢弃前会阻止切换到明确仅支持文本的目标，但已持久化的历史图片不会阻止后续选择；LLM runtime 会在纯文本请求中把它们投影为省略标记。目录成员关系不构成校验：适配器可以解析未列出的模型，而不可用的提供方或不受支持的推理强度会返回 `model-unavailable`。`session.models` 还会报告 `routable`，即当前是否有适配器为所选提供方提供服务。该值刻意不从分组推导，因为适配器可以服务未公布的模型。`session.prompt` 会拒绝向明确不支持图片输入的当前模型新提交图片。
