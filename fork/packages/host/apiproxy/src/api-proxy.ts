...
import { errorChain } from '@deepseek-ai/dsh-fork-llm'
import type { ContentBlock, MessageSource } from '@deepseek-ai/dsh-fork-llm'
...
import type { CallId } from '@deepseek-ai/dsh-fork-llm/brand'
  ...
  return {
    sessions: {
          ...
          try {
            ...
            if (pendingImage) {
              ...
              if (info.inputModalities !== undefined && !info.inputModalities.includes('image')) {
                return err(request, {
                  code: 'model-unavailable',
                  message: `Model "${resolved.model}" cannot be selected while an image prompt is waiting to run.`,
      ...
      updateQueue(request) {
        ...
        if (action.kind === 'edit') {
          agent.inbox.replace(itemId, freezeMessage<typeof message>({ ...message, content: action.content }))
        } else {
        ...
        try {
          const messageId = await ctx.subagents.followup(parent, childSessionId, content, {
            source: {
              ...
              rpcId: request.rpcId,
              ...(canonicalTimeZone === undefined ? {} : { clientTimeZone: canonicalTimeZone }),
            } as Parameters<typeof ctx.subagents.followup>[3]['source'],
            signal,
          })
