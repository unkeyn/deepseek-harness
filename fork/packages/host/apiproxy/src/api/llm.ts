...
export interface DiscoveredModelView {
  ...
  maxTokens?: number
  /**
   * Accepted input modalities per the answering adapter's reference catalogs.
   * Absent means unknown; a surface assumes text-only rather than images.
   */
  inputModalities?: readonly string[]
  /**
   * Reasoning effort levels the model offers per a reference catalog, in
   * escalation order. Absent means unknown or non-reasoning.
   */
