...
import { adoptOfficialLlmErrorPrototype } from './official-error.ts'
...
export class LlmError extends HarnessError {
  ...
  constructor(message: string, code: string, options?: LlmErrorOptions) {
    ...
    adoptOfficialLlmErrorPrototype(LlmError)
    this.name = 'LlmError'
...
export class LlmRuntime extends Service {
  ...
  async discoverModels(
    ...
    for (const model of discovered) {
      ...
      seen.add(model.id)
      // The whole declared row passes through, not a hand-picked subset: the
      // capability fields a discovery implementation enriches from a reference
      // catalog (`inputModalities`, `reasoningLevels`, `catalogMatched`) are
      // part of the reply's meaning, and dropping them would silently turn an
      // enriched answer into a bare listing.
      models.push({
        id: model.id,
