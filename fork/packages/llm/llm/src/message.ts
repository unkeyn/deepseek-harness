...
/** Official plugins extend the source map in their own packages. These structural
 * entries keep fork messages assignable to those consumers without importing the
 * official source module into this package's TypeScript project. */
type OfficialMessageSource =
  | { readonly kind: 'coordinator'; readonly form: 'relay'; readonly senderSessionId: string }
  | { readonly kind: 'subagent-report'; readonly form: 'relay'; readonly senderSessionId: string }
  | { readonly kind: 'subagent-settled'; readonly form: 'notice'; readonly senderSessionId: string }
  | { readonly kind: 'session-reference'; readonly form: 'recall' }
  | { readonly kind: 'goal'; readonly goalId: string; readonly revision: number; readonly round: number }
  | { readonly kind: 'agent-instructions'; readonly form: 'instructions' }
  | { readonly kind: 'skill-catalog'; readonly form: 'catalog' }
  | { readonly kind: 'skill-invocation'; readonly form: 'instructions' }
  | { readonly kind: 'team-message'; readonly teamId: string; readonly messageId: string; readonly senderId: string; readonly senderName: string }
  | { readonly kind: 'user'; readonly rpcId: string; readonly clientTimeZone?: string }
...
export type MessageSource = MessageSourceMap[keyof MessageSourceMap] | OfficialMessageSource
