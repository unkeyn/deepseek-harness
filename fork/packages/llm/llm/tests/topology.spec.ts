...
import LlmRuntime, { LlmAdapter, LlmError } from '@deepseek-ai/dsh-fork-llm'
import type { GenerateOptions, LlmConfigurableProvider, StreamChunk } from '@deepseek-ai/dsh-fork-llm'
...
describe('model discovery registry', () => {
  ...
  it('passes a discovery implementation\'s reference-catalog enrichment through', async () => {
    const ctx = await setup()
    ctx.llm.registerModelDiscovery('llm-example', () => Promise.resolve([
      {
        id: 'enriched',
      ...
      { id: 'unmatched', catalogMatched: false },
    ]))
