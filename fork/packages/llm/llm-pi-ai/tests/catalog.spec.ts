...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@deepseek-ai/dsh-fork-llm'
import type { StreamChunk } from '@deepseek-ai/dsh-fork-llm'
import FileSettingsProvider from '@deepseek-ai/dsh-settings-file'
import ModelCatalog from '@deepseek-ai/dsh-fork-model-catalog'
...
describe('external model capability catalog', () => {
  ...
  it('re-resolves once the identity catalog mounts after this plugin', async () => {
    // A composition may apply this plugin before the catalog: the memo must
    // treat the catalog as an input, or the first — catalog-less — resolution
    // would pin every model's capabilities for the process lifetime.
    const config: LlmPiAi.Config = { providers: { gateway: {
      apiKeyEnv: KEY_ENV,
      api: 'openai-completions',
      baseURL: 'https://gateway.example/v1',
      models: [{ id: 'gpt-5.6-sol' }],
    ...
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmPiAi, config)
    expect((await ctx.llm.resolveModelInfo('gateway', 'gpt-5.6-sol')).reasoning).toBeUndefined()
  ...
  it('keeps a catalog-reference model on its route provider endpoint', async () => {
    // A catalog route listing an id newer than the installed catalog resolves
    // its capability facts from the identity reference — which states no wire
    // address. The route's own provider endpoint must stay authoritative: a
    // reference placeholder of an empty base would send the request to the
    // OpenAI default host, which refuses another tenant's key.
    const ctx = await harness({ providers: { deepseek: {
      apiKeyEnv: KEY_ENV,
      models: [{ id: 'gpt-5.6-sol' }],
    } } }, true)
    const info = await ctx.llm.resolveModelInfo('deepseek', 'gpt-5.6-sol')
    expect(info.reasoning).toBeDefined()
    const resolved = resolveProfiles({
      'deepseek': { models: [{ id: 'gpt-5.6-sol' }] },
    }, ctx.modelCatalog)
    const model = resolved.get('deepseek')?.piProvider.getModels()[0]
    expect(model?.baseUrl).toBe('https://api.deepseek.com')
  })

  it('applies a reference default effort when the request names none', async () => {
    // A gateway that requires an effort serves a specific level when a request
    // names none; a bare request would end in an upstream error or an empty
    // completion. The reference catalog records that default, and the request
    // carries it below an explicit selection.
    ...
    const llmServer = await mockServer([{ events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(ModelCatalog, { refreshUrl: `http://127.0.0.1:${address.port}` })
    ...
    expect((llmServer.requests[0] as { reasoning_effort?: string }).reasoning_effort).toBe('max')

    // An explicit selection still wins over the reference default.
    await assemble(ctx, {
      provider: 'gateway',
      model: 'acme-reasoner',
    ...
    expect((llmServer.requests[1] as { reasoning_effort?: string }).reasoning_effort).toBe('low')
...
describe('hand-declared providers', () => {
  ...
  it('trims paste whitespace off a hand-typed model id and name', () => {
    const resolved = resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
