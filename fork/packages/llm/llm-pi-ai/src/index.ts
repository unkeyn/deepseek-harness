 ...
 * ```
 *
 * @module @deepseek-ai/dsh-fork-llm-pi-ai
 */
...
export function apply(ctx: Context, config: Config): void {
  ...
  let lastCatalog: ModelCatalog | undefined
  let memoized: ReadonlyMap<string, ResolvedPiAiProviderProfile> | undefined
  /**
   * The resolved profiles for the current configuration, memoized by the raw
   * snapshot's identity together with the identity-catalog instance the
   * resolution consumed — which is also what makes the adapter's own snapshot
   * stable across operations that observe no change. The catalog belongs in
   * the key because this plugin may apply before the catalog mounts: keying on
   * the settings alone would pin a snapshot resolved without it, and every
   * model newer than the installed pi-ai catalog would serve without its
   * reasoning and modality facts for the whole process lifetime.
   *
  ...
  const profiles = (): ReadonlyMap<string, ResolvedPiAiProviderProfile> => {
    ...
    if (raw === lastRaw && lastCatalog === catalog && memoized !== undefined) return memoized
