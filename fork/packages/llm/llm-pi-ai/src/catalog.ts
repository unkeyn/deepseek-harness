...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const models = trimmedEntries.map((entry, index) => {
    ...
    // the route's catalog-provider endpoint; an empty string is the reference
    // answer's "no address" spelling and must not win the chain.
    const entryBaseUrl = base?.baseUrl !== undefined && base.baseUrl.length > 0 ? base.baseUrl : undefined
    const baseUrl = request.baseURL ?? entryBaseUrl ?? providerBaseUrl
