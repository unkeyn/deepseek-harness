...
function referenceModel(
  ...
  provider: string,
  id: string,
): { model: Model<Api>; defaultLevel?: ModelThinkingLevel } | undefined {
  // The provider-scoped answer wins: a model id several gateways serve carries
  // provider-specific wire facts, and this route names the gateway it serves.
  const reference = catalog?.resolveFor(provider, id)
  ...
  const model = {
    ...
    // No endpoint: the reference describes the model, never a wire address, so
    // the route's own catalog-provider endpoint stays authoritative. An empty
    // string here would win the baseUrl fallback chain and send the request to
    // the OpenAI default host.
  ...
  // A provider that requires an effort names the one it serves when a request
  // names none; a level outside pi-ai's vocabulary is data drift and no
  // default at all.
  const declared = reference.thinking?.defaultLevel
  const defaultLevel = declared !== undefined && THINKING_LEVEL_GATE[declared as ModelThinkingLevel] === true
    ? declared as ModelThinkingLevel
    : undefined
  return { model, ...defaultLevel === undefined ? {} : { defaultLevel } }
}
...
export interface RouteCatalog {
  ...
  /**
   * The reasoning level a reference catalog names as its provider's default,
   * by model id. A gateway that requires an effort and serves a specific one
   * when a request names none would return an error or an empty answer to a
   * bare request, so the reference's default becomes the request default —
   * below an explicit user selection and below a route-configured level.
   */
  referenceDefaultLevels: ReadonlyMap<string, ModelThinkingLevel>
}
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  // The id is the wire-facing model name, and both it and the display name are
  // hand-typed or adopted from a form, so surrounding paste whitespace travels
  // with them. The provider would answer a padded id with "not a valid model
  // ID" and every capability catalog would miss it, so both fields trim here —
  // the same normalization `normalizeApiKey` applies to keys — and a
  // whitespace-only id then reaches the empty-id refusal below.
  const trimmedEntries = entries.map(entry => ({
    ...entry,
    id: entry.id.trim(),
    ...entry.name === undefined ? {} : { name: entry.name.trim() },
  }))
  ...
  const bases = new Map<string, Model<Api>>()
  const referenceDefaultLevels = new Map<string, ModelThinkingLevel>()
  for (const entry of trimmedEntries) {
    const installed = defaults.get(entry.id)
    if (installed !== undefined) {
      bases.set(entry.id, installed)
      continue
    }
    const reference = referenceModel(request.catalog, provider, entry.id)
    if (reference === undefined) continue
    bases.set(entry.id, reference.model)
    // Only a reference answer carries a provider default effort; an installed
    // entry's own catalog already decided its request behavior.
    if (reference.defaultLevel !== undefined) referenceDefaultLevels.set(entry.id, reference.defaultLevel)
  }
  const models = trimmedEntries.map((entry, index) => {
    ...
    const siblingApi = trimmedEntries.slice(0, index)
      .map(previous => bases.get(previous.id)?.api)
      .find(candidate => candidate !== undefined)
    const api = request.api ?? base?.api ?? routeApi ?? siblingApi ?? GATEWAY_DEFAULT_API
    // An installed or reference entry with no endpoint of its own defers to
    // the route's catalog endpoint; an empty string is the reference
    // answer's "no address" spelling and must not win the chain. Explicit
    // configuration and the provider's declared baseUrl are taken verbatim;
    // only an endpoint derived from per-model siblings is mounted at its
    // version segment.
  ...
  return { models, configuredMaxTokens, referenceDefaultLevels }
}
