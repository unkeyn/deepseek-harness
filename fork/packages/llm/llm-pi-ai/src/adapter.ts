...
export class PiAiAdapter extends LlmAdapter {
  ...
  private prepare(options: GenerateOptions): {
    ...
    const reasoning = resolveReasoningLevel(
      model,
      // Selection order: the request's explicit choice, the route-configured
      // level, then the reference catalog's provider default — a gateway that
      // requires an effort answers a bare request with an error or an empty
