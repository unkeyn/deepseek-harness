# @deepseek-ai/dsh-fork-web-fetch-http

An anonymous public HTTP(S) `WebFetchProvider` for the fork [web capability seam](../web/README.md) (`ctx.web`). It retrieves a concrete URL and returns a status code plus bounded decoded content.
...
The provider's `timeoutMs` is a resource backstop for direct `ctx.web.fetch()` callers and misconfigured deployments, not the model-facing tool-call budget. [`dsh-tool-call-timeout-policy`](../../../../packages/guard/timeout-policy/README.md) owns the `web_fetch` tool-call budget by arming `exec.signal`.
...
- Follows only **same-origin** redirects; a cross-origin redirect fails with `WEB_REDIRECT_BLOCKED`, requiring a fresh tool call.
...
| `userAgent` | `deepseek-harness/0.0.1` | `User-Agent` header. |
...
Indirectly, through `dsh-tool-web`, which places this provider's `maxBodyChars`-bounded decoded text or markdown-shaped HTML under its fetch-result wrapper and retains provider failures while redirects, headers, and transport mechanics remain hidden. The fork composition mounts this provider beside the official `tool-web` row replacement whose config keeps `web_fetch` enabled, so the model can read any source a search result cites.
