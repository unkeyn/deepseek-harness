...
describe('WebRuntime execution resolution', () => {
  ...
  it('contributes the research-methodology section when the system-prompt service is mounted', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(WebRuntime, { searchProviders: ['exa'] })
    const assembly = await ctx.systemPrompt.assemble()
    expect(assembly.sections.map(section => section.name)).toContain('web:research')
  })
