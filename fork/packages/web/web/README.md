...
| Package | Role |
|---|---|
| `@deepseek-ai/dsh-fork-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@deepseek-ai/dsh-fork-web-search-brave` | Search provider: Brave |
| `@deepseek-ai/dsh-fork-web-search-firecrawl` | Search provider: Firecrawl |
| `@deepseek-ai/dsh-fork-web-search-pool` | Search provider: user-configured generic HTTP pool with per-key rotation (`custom-pool`) |
| `@deepseek-ai/dsh-fork-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
...
Selection never depends on registration, config, or HMR order. A capability has an explicit route or auto-selects when exactly one usable provider is registered. For search, `searchProviders` names an ordered fallback route tried first-to-last; a route member that is registered but unavailable is skipped, and a member that throws moves execution to the next entry (cancellation excepted). Single-id config (`searchProvider`/`fetchProvider`, or env `$DSH_WEB_SEARCH_PROVIDER[S]`/`$DSH_WEB_FETCH_PROVIDER` feeding the same fields) is a one-entry route. `search()`/`fetch()` resolve at execution time:

| Situation | Execution |
|---|---|
| configured id registered and `available()` | runs that provider |
| configured id not registered | `WEB_PROVIDER_CONFIGURED_MISSING` |
| configured id registered but unavailable | skipped on a multi-entry route; otherwise `WEB_PROVIDER_CONFIGURED_UNAVAILABLE` |
...
Indirectly, through `dsh-tool-web`, which retains bounded normalized provider data or the exact configured-provider, unavailable-provider, no-provider, multiple-provider, and `Error: <message>` failures. The service also contributes one provider-agnostic system-prompt section (`web:research`) stating the research methodology — search, then verify claims by fetching the cited pages, prefer and corroborate primary sources, never re-search a known URL, mark unverified claims. It assumes the composition exposes both `web_search` and `web_fetch` (every shipped fork composition does); a composition that drops a tool owns replacing this section.

#### KV Cache effect

The `web:research` section adds its fixed text to every model request prefix when the system-prompt service is mounted; everything else is owned by the named consumer.
