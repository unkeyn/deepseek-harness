...
describe('credential broker memory through a real Loader composition', () => {
  it('mounts configured entries and exposes only pool metadata', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@deepseek-ai/dsh-credential-broker-memory'",
      '  config:',
      '    entries:',
      '      - pool: main',
      '        credential: key-a',
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@deepseek-ai/dsh-credential-broker-memory') return { apply: (await import('../src/index.ts')).apply, MemoryCredentialBroker }
