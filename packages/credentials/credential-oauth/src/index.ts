...
export class OAuthLifecycle {
  ...
  private async refreshAccount(account: Account): Promise<string> {
    if (account.refreshRef === undefined) return this.markReauthenticateIfCurrent(account, 'provider does not supply a refresh credential')
  ...
  private references(accountId: string, includeRefresh: boolean): { accessRef: CredentialRef; refreshRef?: CredentialRef } {
