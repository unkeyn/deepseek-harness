...
import { credentialId } from '@deepseek-ai/dsh-credential-broker'
...
describe('OAuthLifecycle', () => {
  ...
  it('projects generation-aware broker metadata without token values', async () => {
    let current: RemoteOAuthCredentialSnapshot = {
      generation: 2,
      credentials: [
        { id: credentialId('oauth-account'), provider: 'test', reference: credentialRef('oauth-ref'), authKind: 'oauth', accountId: 'account-1' },
        { id: credentialId('api-key'), provider: 'test', reference: credentialRef('api-ref'), authKind: 'api-key' },
      ],
    }
    ...
    current = {
      generation: 3,
      credentials: [{ ...current.credentials[0]!, accountId: 'other-account', reference: credentialRef('new-oauth-ref') }, current.credentials[1]!],
    }
    ...
    await expect(store.set(credentialRef('oauth-ref'), 'token')).rejects.toMatchObject({ code: 'OAUTH_REMOTE_STORE_READ_ONLY' })
    await expect(store.unset(credentialRef('oauth-ref'))).rejects.toMatchObject({ code: 'OAUTH_REMOTE_STORE_READ_ONLY' })
    await expect(store.resolve(credentialRef('oauth-ref'))).resolves.toBeUndefined()
  })

  it('applies broker snapshot events and disposes the subscription', () => {
    let listener: ((event: import('@deepseek-ai/dsh-credential-broker').CredentialBrokerSnapshotEvent) => void) | undefined
    let disposed = false
    const source = {
    ...
    listener?.({ kind: 'entry', generation: 2, entry: { id: credentialId('oauth'), pool: 'main' as never, provider: 'test', reference: credentialRef('oauth-ref'), authKind: 'oauth', accountId: 'account-1' } })
