# Agent Note: Classify transient pi-ai failures from flattened message text
...
A TUI run whose model connection dropped mid-stream surfaced the single notice `terminated`, and a truncated Anthropic response surfaced `Anthropic stream ended before message_stop`. Both are transport truncations — the connection died before the provider's terminal SSE event — yet `classifyPiAiError` in `dsh-llm-pi-ai` mapped neither, falling through to the catch-all `PI_AI_ERROR`. A rotating gateway also reported temporary supplier unavailability as either `Our servers are currently overloaded` or an HTTP 400 body carrying `type: upstream_unavailable`; the classifier treated the first as `PI_AI_ERROR` and the second as `INVALID_REQUEST`. Because none of those codes are in `llm-retry`'s `DEFAULT_RETRYABLE_CODES` (`RATE_LIMIT`, `SERVER`, `TIMEOUT`, `TRANSPORT`), recoverable failures were treated as permanent and never retried.
...
- The classifier maps explicit temporary-availability wording (`upstream_unavailable` and `servers are currently overloaded`) to `SERVER` before generic HTTP 400 matching. The semantic signal takes precedence because a rotating gateway may encode an upstream supplier outage inside an otherwise permanent-looking status code.
...
- A mid-stream transport drop and a pre-terminal stream truncation now carry `TRANSPORT`; explicit gateway overload and upstream-unavailable messages carry `SERVER`. A composed `llm-retry` policy therefore retries each of them by default instead of failing the turn.
